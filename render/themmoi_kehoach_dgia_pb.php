    ...
    <tbody>
    	<?php if ($dep_id!=0): ?>
    		<?php $stt=0; foreach ($data_list_nv as $key => $value): ?>
    			<?php if ($value['dep_id']==$dep_id): $stt++;?>
    				<tr class="nv_chung_dc_ng_dg nv_id_dc_ng_dg<?=$value['ep_id']?>">
			            <td>
			                <p><?=$stt?></p>
			            </td>
			            <td>
			            ...
			            <td>
			                <input <?if(in_array($value['ep_id'], $str_id_nv)==true) echo "checked"; ?> type="checkbox" class="checkcon un_check_con_<?=$value['ep_id'];?>" value="<?=$value['ep_id'];?>">
			            </td>
	        		</tr>
    			<?php endif ?>
    		<?php endforeach ?>
    	...
    	<?php if ($dep_id==0): ?>
    		<?php $stt=0; foreach ($data_list_nv as $key => $val):$stt++ ?>
    			<tr class="nv_chung_dc_ng_dg nv_id_dc_ng_dg<?=$val['ep_id']?>">
		            <td>
		                <p><?=$stt?></p>
		            </td>
		            <td>
