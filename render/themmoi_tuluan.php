...
<?php $stt=0; foreach ($tuluan as $value): ?>
	<?php if (in_array($value['id'],$str_id)):$stt++; ?>
		<tr class="row_table chungchungtl">
			<td><?=$stt?></td>
			<td  >
				<?$value['cauhoi'] = str_replace('<br />', '', $value['cauhoi'] );?>
				<p style="width: 580px;" class="text_a_l one_line2"><?=$value['cauhoi']?></p>
			</td>
			<td class="tongcontl"><?=$value['sodiem']?></td>
			<td>
				<div data-id-ch='<?=$value['id']?>' onclick="del_row_tuluan(this)" class="btn_xoa d_flex c-pointer j_center hieuung">
					<div class="img mr_5">
...
$(document).ready(function(){
    var tong_d_tc= 0;
    $('.tongcontl').each(function(){
        tong_d_tc+=Number($(this).text());
    })
    $('.dang_tuluan .show_tong_tracnghiem').text(tong_d_tc);
})
</script>